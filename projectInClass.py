        ...
        if event.type() == QtCore.QEvent.KeyPress and obj is self.passTextEdit:
            if event.key() == QtCore.Qt.Key_Return:
                if self.passTextEdit.toPlainText() == "" or self.nameTextEdit.toPlainText() == "":
                    pass
                elif self.passTextEdit.toPlainText().isalpha():
                    self.createUser()
                elif len(self.results) == 1:
                    self.login()
                return True
...
class Window(QMainWindow):
    ...
    def initUI(self):
        ...
        self.RVlayout.addStretch()

        self.textLabel = QtWidgets.QLabel(self, wordWrap=True, alignment=QtCore.Qt.AlignmentFlag.AlignCenter)
        ...
        if len(self.inputContent) > 0:
            ...
            if self.inputContent != self.text[:len(self.inputContent)]:
                ...
                if not (self.firstlen > self.secondlen): #adding to wrongChar if user is wrong and not backspacing
                    self.wrongChar += 1
                self.textLabel.setText("<font color='#D1D0C5'>{}</font><font color='red'><u>{}</u></font>{}".format(self.correctInput, self.wrongInput, self.text[len(self.correctInput)+len(self.wrongInput):]))
                ...
                if len(self.inputContent) > len(self.correctInput):
                    ...
                    self.wrongInput = ""
                self.textLabel.setText("<font color='#D1D0C5'>{}</font>{}".format(self.correctInput, self.text[len(self.correctInput):]))
            self.setWPM()
            self.setAccu()
